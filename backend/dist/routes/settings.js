    ...
    try {
        ...
        if (page) {
            ...
            if (setting && setting.settingValue) {
                ...
                res.json(seoConfigDefaults[page] || {});
            }
        }
        else {
            ...
            const result = { ...seoConfigDefaults };
            settings.forEach(s => {
...
router.get('/general', async (req, res) => {
    try {
        ...
        if (!result.site_title)
            result.site_title = 'GeTaToS Safari - Ihre Safari Experten';
        if (!result.default_language)
            result.default_language = 'de';
        if (!result.currency)
            result.currency = 'EUR';
        if (!result.timezone)
            result.timezone = 'Africa/Dar_es_Salaam';
        if (result.booking_enabled === undefined)
            result.booking_enabled = true;
        res.json(result);
    }
    catch (err) {
...
router.get('/analytics', async (req, res) => {
    try {
        ...
        if (!result.gtm_id)
            result.gtm_id = '';
        if (!result.gdpr_cookie_banner)
            result.gdpr_cookie_banner = false;
        res.json(result);
    }
    catch (err) {
