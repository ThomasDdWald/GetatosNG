...
// Custom request type with admin property
...
function generateTOTPUri(secret: string, label: string): string {
...
const authenticateAdmin = async (req: AuthenticatedRequest, res: Response, next: () => void) => {
...
let fallbackUsers: Array<{email: string; password: string; secret?: string; full_name?: string}> = []
...
router.post('/2fa/setup', authenticateAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.post('/2fa/verify', authenticateAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { code } = req.body
    const admin = (req as any).admin
    const email = admin.email
...
// Disable 2FA
router.post('/2fa/disable', authenticateAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { code } = req.body
    const admin = (req as any).admin
    const email = admin.email
...
router.get('/2fa/status', authenticateAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const admin = (req as any).admin
    const email = admin.email
...
router.post('/login-with-2fa', async (req, res) => {
  ...
  try {
    ...
    const decoded = jwt.verify(twofaToken, effectiveJwtSecret) as any
...
router.post('/posts', authenticateAdmin, async (req, res) => {
  try {
    ...
    const { 
      featured_image, featuredImage, author, is_published, isPublished,
      // Alle Sprachen - camelCase (titleDe, contentDe, etc.)
      titleDe, titleEn, titleEs, titleFr, titleZh,
      contentDe, contentEn, contentEs, contentFr, contentZh,
      excerptDe, excerptEn, excerptEs, excerptFr, excerptZh,
      slugDe, slugEn, slugEs, slugFr, slugZh,
      // Category
      categoryDe, categoryEn, categoryEs, categoryFr, categoryZh,
      // SEO
      metaTitleDe, metaTitleEn, metaTitleEs, metaTitleFr, metaTitleZh,
      metaDescriptionDe, metaDescriptionEn, metaDescriptionEs, metaDescriptionFr, metaDescriptionZh,
      h1De, h1En, h1Es, h1Fr, h1Zh,
      featuredImageAltDe, featuredImageAltEn, featuredImageAltEs, featuredImageAltFr, featuredImageAltZh
    } = req.body
...
router.post('/tours', authenticateAdmin, async (req, res) => {
  try {
    const { 
      titleEn, titleDe, titleEs, titleFr, titleZh,
      descriptionEn, descriptionDe, descriptionEs, descriptionFr, descriptionZh,
      shortDescriptionEn, shortDescriptionDe, shortDescriptionEs, shortDescriptionFr, shortDescriptionZh,
      // SEO meta fields
      metaTitleEn, metaTitleDe, metaTitleEs, metaTitleFr, metaTitleZh,
      metaDescriptionEn, metaDescriptionDe, metaDescriptionEs, metaDescriptionFr, metaDescriptionZh,
      slugEn, slugDe, slugEs, slugFr, slugZh,
      tourType, durationDays, difficulty, priceEn, priceDe, priceEs, priceFr, priceZh, 
      currencyEn, currencyDe, currencyEs, currencyFr, currencyZh, maxGroupSize,
      highlightsEn, highlightsDe, highlightsEs, highlightsFr, highlightsZh,
      includedEn, includedDe, includedEs, includedFr, includedZh,
      notIncludedEn, notIncludedDe, notIncludedEs, notIncludedFr, notIncludedZh,
      isFeatured, isActive,
      startLat, startLng, startName, endLat, endLng, endName, waypoints,
