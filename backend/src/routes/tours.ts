...
router.get('/', async (req, res) => {
  try {
    ...
    if (type && type !== 'all') where.tourType = type
    ...
    const mapped = tours.map(t => ({
      id: t.id,
      title: lang ? getLocalized(t, 'title', lang as string) : t.titleEn,
      ...
      description: lang ? getLocalized(t, 'description', lang as string) : t.descriptionEn,
      ...
      short_description: lang ? getLocalized(t, 'shortDescription', lang as string) : t.shortDescriptionEn,
      ...
      // SEO meta fields
      metaTitle: lang ? getLocalized(t, 'metaTitle', lang as string) : t.metaTitleEn,
      ...
      metaDescription: lang ? getLocalized(t, 'metaDescription', lang as string) : t.metaDescriptionEn,
      ...
      featured_image: (t as any).images?.[0]?.url || null,
      images: (t as any).images,
...
router.get('/featured', async (req, res) => {
  try {
    ...
    console.log("Finding tours..."); const tours = await prisma.tour.findMany({
...
router.get('/:slug', async (req, res) => {
  try {
    ...
    const mapped = {
      id: tour.id,
      title: lang ? getLocalized(tour, 'title', lang as string) : tour.titleEn,
      ...
      description: lang ? getLocalized(tour, 'description', lang as string) : tour.descriptionEn,
      ...
      short_description: lang ? getLocalized(tour, 'shortDescription', lang as string) : tour.shortDescriptionEn,
      ...
      // SEO meta fields
      metaTitle: lang ? getLocalized(tour, 'metaTitle', lang as string) : tour.metaTitleEn,
      ...
      metaDescription: lang ? getLocalized(tour, 'metaDescription', lang as string) : tour.metaDescriptionEn,
      ...
      highlights: lang ? getLocalizedJson(tour, 'highlights', lang as string) : tour.highlightsEn, 
      included: lang ? getLocalizedJson(tour, 'included', lang as string) : tour.includedEn, 
      not_included: lang ? getLocalizedJson(tour, 'notIncluded', lang as string) : tour.notIncludedEn,
      is_featured: tour.isFeatured, is_active: tour.isActive,
      images: (tour as any).images.map((img: any) => ({ id: img.id, tour_id: img.tourId, url: img.url, alt_text: img.altText, is_cover: img.isCover, display_order: img.displayOrder })),
      itinerary: (tour as any).itinerary.map((it: any) => ({ 
        id: it.id, 
        tour_id: it.tourId, 
...
function getLocalized(tour: any, field: string, lang: string): string {
  const fieldMap: Record<string, string> = { de: field + 'De', en: field + 'En', es: field + 'Es', fr: field + 'Fr', zh: field + 'Zh' }
  return tour[fieldMap[lang] || field] || tour[field] || ''
}

function getLocalizedJson(tour: any, field: string, lang: string): any[] {
  const fieldMap: Record<string, string> = { de: field + 'De', en: field + 'En', es: field + 'Es', fr: field + 'Fr', zh: field + 'Zh' }
  return tour[fieldMap[lang] || field] || tour[field] || []
}
