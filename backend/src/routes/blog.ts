...
router.get('/', async (req, res) => {
  try {
    ...
    const suffix = langMap[lang as string] || 'En'
    
    const localizedPosts = posts.map(post => {
      // Get localized field - prefers language-specific field, falls back to base
      const getField = (field: string, suffix?: string) => {
        // Try localized field first (e.g. titleDe)
        const localized = suffix ? (post as any)[`${field}${suffix}`] : null
        if (localized) return localized
        // Fallback to base field (e.g. title, title_de)
        return (post as any)[field] || (post as any)[`${field}_${lang}`] || null
      }
      
      return {
        id: post.id,
        slug: getField('slug', suffix) || post.slug,
        title: getField('title', suffix) || post.title || (post as any).title_de || '',
        content: getField('content', suffix) || post.content || (post as any).content_de || '',
        excerpt: getField('excerpt', suffix) || post.excerpt || (post as any).excerpt_de || '',
...
router.get('/:slug', async (req, res) => {
  try {
    ...
    if (lang) {
      ...
      const langField = langMap[lang as string]
      if (langField) {
    ...
    const localizedPost = {
      id: post.id,
      slug: suffix ? (post as any)[`slug${suffix}`] : post.slug,
      title: suffix ? (post as any)[`title${suffix}`] : post.title,
      content: suffix ? (post as any)[`content${suffix}`] : post.content,
      excerpt: suffix ? (post as any)[`excerpt${suffix}`] : post.excerpt,
      featuredImage: post.featuredImage,
      featuredImageAlt: suffix ? (post as any)[`featuredImageAlt${suffix}`] : null,
      author: post.author,
      category: suffix ? (post as any)[`category${suffix}`] : null,
      isPublished: post.isPublished,
      createdAt: post.createdAt,
      // SEO
      metaTitle: suffix ? (post as any)[`metaTitle${suffix}`] : null,
      metaDescription: suffix ? (post as any)[`metaDescription${suffix}`] : null,
      h1: suffix ? (post as any)[`h1${suffix}`] : null,
      // Include all language fields for frontend
      slug_de: post.slugDe || null,
      slug_en: post.slugEn || null,
      slug_es: post.slugEs || null,
