...
import { initFirebase, verifyIdToken } from './services/firebaseAuth.js'
...
async function loadRoute(name: string, routePath: string): Promise<void> {
  try {
    ...
    const message =
      error instanceof Error ? error.message : 'Unknown route load error'
